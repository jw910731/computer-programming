...
int main(int argc, char **argv) {
    ...
    while(c != -1){
        switch((c = getopt_long(argc, argv, "a:i:o:h", lopts, NULL))){
            ...
            case 'i': {
                ...
                inputFileName = calloc(siz + 1, sizeof(char));
            ...
            case 'o':
                ...
                size_t siz = strlen(optarg);
                outputFileName = calloc(siz + 1, sizeof(char));
    ...
    fp rot_deg = -1 * PI * in_deg / 180.0;

    // argparse error check
    if(outputFileName == NULL){
        fputs("No output file name provided! \n", stderr);
        return 1;
    }

    if(inputFileName == NULL){
        fputs("No input file name provided!\n", stderr);
        return 1;
    }

    if(in_deg < 0){
        fputs("No angle argument provided!\n", stderr);
        return 1;
    }
    ...
    if ((inBmp = fopen(inputFileName, "rb")) == NULL) {
        perror("Input file could not be opened");
        return 1;
    }

    if ((outBmp = fopen(outputFileName, "wb+")) == NULL) {
    ...
    struct vec_t diag[2] = {rotate(sub((struct vec_t){.x = 0, .y = inPic.h},
                            ...
                            rotate(sub((struct vec_t){.x = 0, .y = 0},
                                   ...
                                   rot_deg)};
    i32 san_oh = (i32)ceil(MAX(fabs(diag[0].y), fabs(diag[1].y))) + 1,
        san_ow = (i32)ceil(MAX(fabs(diag[0].x), fabs(diag[1].x))) + 1;
    //printf("ow: %d oh: %d\n", san_ow, san_oh);
